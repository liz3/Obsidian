...
namespace Obsidian.Api.IO.WAD
{
    ...
    public class WADFile : IDisposable
    {
        ...
        public WADEntry AddEntry(string path, byte[] data, bool compressedEntry)
        {
            using (XXHash64 xxHash = XXHash64.Create())
            {
                return AddEntry(BitConverter.ToUInt64(xxHash.ComputeHash(Encoding.ASCII.GetBytes(path.ToLower(new CultureInfo("en-US")))), 0), data, compressedEntry);
            }
        }
        ...
        /// <param name="data">Data of file being added</param>
        /// <param name="compressedEntry">Whether the data needs to be GZip compressed inside WAD</param>
        public WADEntry AddEntry(ulong xxHash, byte[] data, bool compressedEntry)
        {
            var newEntry = new WADEntry(this, xxHash, data, compressedEntry);
            AddEntry(newEntry);
